...
module Tezos = struct
  ...
  module Make (C : CRYPTO) = struct
    ...
    let to_base58 { version; payload } = of_bytes (Version.to_raw_bytes version ^ payload)
...
module Bitcoin = struct
  ...
  module Make (C : CRYPTO) = struct
    ...
    let t_of_bytes s =
      ...
      | '\x00' :: _ -> Version.P2PKH, String.sub s 1 (len - 1)
...
module Komodo = struct
  ...
  module Make (C : CRYPTO) = struct
    ...
    let version_of_bytes_exn s =
      let len = String.length s in
      match chars_of_string s with
      | '\060' :: _ -> Version.P2PKH, String.sub s 1 (len - 1)
